...
# logging
logging.config.fileConfig(fname="/app/yaget/management/commands/wowma_do_logispi_order_logging.config", disable_existing_loggers=False)
...
class Command(BaseCommand):
    def __init__(self):
        ...
        help = 'do update order csv to logispi.'
        self.logger.debug('wowma_do_logispi_order Command in. init')
    ...
    def handle(self, *args, **options):
        ...
        try:
            self.logger.info('wowma_do_logispi_order handle is called')
            self.batch_status = BatchStatusUpd('wowma_do_logispi_order')
            self.batch_status.start()
            self.wowma_access = WowmaAccess(self.logger)

            # ロジスピに入荷用CSVをアップロードする母体
            self._exec_logispi_order(options['pk'], options['payment_method'])

            self.logger.info('wowma_do_logispi_order handle end.')
    ...
    # wowmaから注文情報を取り込みロジスピの入荷用CSVをアップロードする
    def _exec_logispi_order(self, pk, payment_method):

        # 受注番号をコマンド引数 pk から取得。とれただけループして確認する
        self.logger.info('wowma_do_logispi_order _exec_logispi_order handle pk:' + pk)
        ...
        if wow_order:
            ...
            self.logger.info('wowma_do_logispi_order _exec_buyers_order ログイン成功まで')
            ...
            # 注文がヒットなし
            self.logger.info('wowma_do_logispi_order _exec_buyers_order 該当の注文が見つからないので処理終了[{}]'.format(pk))
