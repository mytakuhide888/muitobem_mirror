...
- 下記の「市場で売れやすい悩みの切り口」を必ず1案につき1つ以上含めること:
  1) 彼の本音特化
  2) 復縁/音信不通特化
  3) 不倫/複雑愛特化
  4) 連絡時期/会える時期特化
  5) 縁結び/思念伝達/波動修正付き
  6) 具体アドバイス付き伴走型
- 5案のうち最低3案は「相手の気持ち × 時期 × 具体アクション」を明示的に統合すること
- タイトル/コンセプト一言は広い看板語を避け、悩みをピンで刺す具体語にすること
  （例: 「恋愛鑑定」ではなく「彼の本音を視る」「連絡が来る日と送るべきLINEを伝える」）
...
## キャラクターが伝えるメッセージ骨子
- 次の3軸を必ず含むメッセージ骨子を3パターン作る:
  1) 相手の気持ち（何をどう読むか）
  2) 時期（いつ動くか/待つか）
  3) 具体アクション（送るLINE、行動、避ける行動）
- それぞれを「刺さる一言」「説明（2〜3文）」「具体例」の形式で書く
- 可能なら次の売れ筋切り口を織り込む:
  彼の本音 / 復縁・音信不通 / 不倫・複雑愛 / 連絡時期・会える時期 / 縁結び・波動修正 / 伴走型アドバイス
