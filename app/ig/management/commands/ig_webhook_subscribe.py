...
def _app_token(app_id: str) -> str:
    secret = getattr(settings, "META_IG_APP_SECRET", "") or getattr(settings, "META_APP_SECRET", "")
    if not (app_id and secret):
        raise CommandError("META_IG_APP_ID / META_IG_APP_SECRET が未設定です。")
    return f"{app_id}|{secret}"


def _public_origin() -> str:
    # 環境変数優先 → settings から推定
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--fields", default="comments,messages,mentions",
                            help="カンマ区切り。既定は comments,messages,mentions")
        parser.add_argument("--app-id", default=None,
                            help="使うApp ID（既定: settings.META_IG_APP_ID または META_APP_ID）")
        parser.add_argument("--verify-token", default=None,
                            help="既定: settings.META_WEBHOOK_VERIFY_TOKEN")

    def handle(self, *args, **opts):
        app_id = opts["app_id"] or getattr(settings, "META_IG_APP_ID", None) or getattr(settings, "META_APP_ID", None)
        if not app_id:
            raise CommandError("META_IG_APP_ID または META_APP_ID が未設定です。")
