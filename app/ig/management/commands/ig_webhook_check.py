...
def _app_token(app_id: str) -> str:
    """
    app_id に対応する app secret を環境設定から取得する。
    META_APP_* / META_IG_APP_* を探索して組み立てる。
    """
...
def _list_subscriptions(app_id: str) -> dict:
    try:
        j = metaapi.api_get(f"{app_id}/subscriptions", params={"access_token": _app_token(app_id)})
        return j or {"data": []}
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **opts):
        ...
        app_ids = []
        for key in ("META_APP_ID", "META_IG_APP_ID"):
        ...
        for aid in app_ids:
            ...
            data = j.get("data", [])
            entries = [{"object": d.get("object"), "fields": d.get("fields", [])} for d in data]
